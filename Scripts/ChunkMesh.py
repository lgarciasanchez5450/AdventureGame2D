...
class ChunkMesh:
    ...
    def __init__(self,ctx:mgl.Context,program:mgl.Program,chunk:Chunk) -> None:
        ...
        #     return
        # self.vbo = self.ctx.buffer(build_mesh(self.chunk.blocks))
        # self.vao = self.ctx.vertex_array(self.program,[(self.vbo,'3i i','in_position','in_face')])
    ...
    def buildMesh(self):
        ...
        self.vao = self.ctx.vertex_array(self.program,[(self.vbo,'3i i i','in_position','in_face','in_id')])
...
def build_mesh(b:np.ndarray,lod:int):
    ...
    elif (Chunk.SIZE/lod).is_integer():
...
def _build_mesh(b:np.ndarray,chunk_size):
    ...
    # #along the y-axis bitmasks
    # a = np.roll(b,-1,axis=1)
    # a[-1] = 0
    # ypos = np.logical_and(np.logical_not(a),b)
    # a = np.roll(b,1,axis=1)
    # a[0] = 0
    # yneg = np.logical_and(np.logical_not(np.roll(b,1,axis=1)),b)
