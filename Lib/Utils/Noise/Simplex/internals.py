...
@njit(cache=True,parallel=True)
def _noise2a_ip(x, y, perm, out):
    for y_i in prange(y.size):
        for x_i in prange(x.size):
            out[y_i, x_i] = _noise2(x[x_i], y[y_i], perm)
    return out
...
@njit(cache=True,parallel=True)
def _noise3a_ip(x, y, z, perm, perm_grad_index3, out):
    for z_i in prange(z.size):
        for y_i in prange(y.size):
            for x_i in prange(x.size):
                out[z_i, y_i, x_i] = _noise3(x[x_i], y[y_i], z[z_i], perm, perm_grad_index3)
    return out
...
@njit(cache=True, parallel=True)
def _noise4a_ip(x, y, z, w, perm, out):
    for w_i in prange(w.size):
        for z_i in prange(z.size):
            for y_i in prange(y.size):
                for x_i in prange(x.size):
                    out[w_i, z_i, y_i, x_i] = _noise4(x[x_i], y[y_i], z[z_i], w[w_i], perm)
    return out
