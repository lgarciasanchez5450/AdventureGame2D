...
def decompress(b:bytes):
    ...
    while i < b.__len__():
        ...
        # if cb_len == 0xFF or cb_len == 0xFF_FF or cb_len == 0xFF_FF_FF:
...
if __name__ == '__main__':
    ...
    print(f'Size of Test Data: {len(bytes(test))}')
    test = b'dtype=uint16,blocks=0,entities=1,sbytessize=4\n\r,\x00*/Entity:\x00\x08\x00\x00\x00\x00\x00\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x00\x00\x08\x00\x00\x80?\x00\x00\x80?\x03\x00\x00\x02\x011'
    c = b"\x00d\x00t\x00y\x00p\x00e\x00=\x00u\x00i\x00n\x021\x006\x00,\x00b\x00l\x00o\x00c\x00k\x00s\x060\x0ce\tt\x08t\x08e\x12=\x001\x0cs\r\ny\x02e\x12s\x08z\x05=\x004\x00\r\n\x00\r\n\x00\x00#\x00\x00\xef\xbf\xbd\x00\x02#\x01'\x00\x00\x01(\x01*\x00&\x00,\x02+\x01+\x02#\x020\x00-\x00)\x003\x011\x025\x002\x02(\x026\x029\x007\x00;\x02:\x02=\x00<\x00?\x021\x00$,#*\x00/\x00E\x15i\x02y\x00:#\x08$\x00J\x00I\x00K\x00$\x08$\xef\xbf\xbd\x00?O?\x00\x03$\x02)1"
    compressed = bytes(compress(test))
    print(compressed)
    print(c)
    print(c==compressed)
    # print(decompress(comp))
    # decomp_jit(compressed)
    right = 0
    wrong = 0
    wrongs:list[int] = []
    ...
    compressed = bytes(compress(t))

    b = len(test)
    from timeit import timeit
    time = timeit('compress(t)',number=1_000,globals=globals())
    dtime = timeit('decompress(compressed)',number=1_000,globals=globals())
    ztime = timeit('zlib.compress(t,9)',number=1_000,globals=globals())

    dztime = timeit('zlib.decompress(zcompressed)',number=1_000,globals=globals())

    
    print(f'Compression Speed: {(b/(time*1000)):.2f} MB/s')
    print(f'ZLIB Compression Speed: {(b/(ztime*1000)):.2f} MB/s')

    print(f'Decompression Speed: {(b/(dtime*1000)):.2f} MB/s')
    print(f'ZLIB Decompression Speed: {(b/(dztime*1000)):.2f} MB/s')
