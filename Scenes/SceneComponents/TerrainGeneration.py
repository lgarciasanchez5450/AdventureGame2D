...
def combine_callbacks(terrain,humidity,temperature):
    def _(a:typing.Mapping[tuple[int,int],tuple[terrainType,humidityType,temperatureType]]):
        t = {}
        h = {}
        t2 = {}
...
class WorldGen(PipelineLayerMultiStep[tuple[int,int],tuple[terrainType,humidityType,temperatureType]]):
    ...
    def __init__(self,chunk_size:int,
        ...
        super().__init__(combine_callbacks(terrain,humidity,temperature))
        self.chunk_size = chunk_size
        assert context is not None
    ...
    def step(self, cpos: tuple[int, int]) -> typing.Generator[None, None, tuple[terrainType, humidityType, temperatureType]]:
        xs = np.arange(self.chunk_size) + cpos[0] * self.chunk_size
        ys = np.arange(self.chunk_size) + cpos[1] * self.chunk_size
        ...
        humidity_offset /= (1+abs(self.sea_level))**2
        humidity = rainfall - humidity_offset

        return WorldGen.height_to_block(heightmap),float(np.average(humidity)),float(np.average(temperature))